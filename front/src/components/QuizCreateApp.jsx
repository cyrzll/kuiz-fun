...
export default function QuizCreateApp() {
  ...
  const [materials, setMaterials] = useState([
    { title: 'Sila Ke-1: Ketuhanan Yang Maha Esa', content: 'Sila pertama melambangkan keyakinan bangsa Indonesia terhadap Tuhan. Nilai-nilainya mencakup toleransi antarumat beragama dan kebebasan menjalankan ibadah sesuai keyakinan masing-masing.', icon: '🇮🇩' }
  ]);

  const [questions, setQuestions] = useState([
    {
      question: 'Apa lambang dari sila pertama Pancasila?',
      explanation: 'Lambang sila pertama Pancasila adalah Bintang Emas segi lima di bagian tengah perisai Garuda Pancasila.',
      options: [
        { key: 'A', text: 'Bintang', isCorrect: true },
        { key: 'B', text: 'Rantai', isCorrect: false },
        { key: 'C', text: 'Pohon Beringin', isCorrect: false },
        { key: 'D', text: 'Kepala Banteng', isCorrect: false }
      ]
    }
  ]);
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const res = await fetch(getBackendUrl('/api/modules'), {
        method: 'POST',
        headers: {
      ...
      if (!res.ok || data.error) {
        throw new Error(data.error || 'Gagal menyimpan modul kuis');
      }

      showModal({
        title: 'Kuis Berhasil Dibuat!',
        message: `Kuis "${title}" telah berhasil disimpan ke database. Anda dapat memulai room kuis ini sekarang dari dashboard.`,
        type: 'success',
        icon: '🎉',
        onConfirm: () => {
      ...
      showModal({
        ...
        message: 'Gagal menghubungi server untuk menyimpan kuis kustom: ' + err.message,
        type: 'error',
        icon: '❌'
      });
    } finally {
    ...
    return (
      <div className="min-h-screen bg-[#FDFBF7] bg-grid-pattern py-8 px-4 flex items-center justify-center">
        <div className="max-w-md w-full neo-box bg-white p-8 text-center space-y-4">
          ...
          <p className="text-sm font-bold text-gray-700 font-poppins">Memeriksa autentikasi...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto space-y-8 animate-fadeIn">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center bg-white neo-box p-4 sm:px-6 sm:py-4 gap-4">
          <div className="flex items-center gap-3">
            <span className="text-3xl sm:text-4xl bg-neo-purple p-1.5 neo-border shadow-sm text-white">🎨</span>
            <div>
              <h1 className="text-xl sm:text-2xl font-black uppercase tracking-tight text-black">
                PEMBUAT <span className="bg-neo-purple text-white px-1.5 neo-border shadow-sm border-black">MODUL & KUIS</span>
              </h1>
              <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mt-0.5">
                Rancang Materi Edukasi dan Kuis Interaktif
              </p>
            </div>
          </div>
          <a
